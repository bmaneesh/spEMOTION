...
class_num = targets.max()
targets_onehot = np.zeros((targets.shape[0], class_num+1))
...
class lstm_att(nn.Module):
    ...
    def forward(self, input, istraining=False):
        ...
        # print contxt.size()
        out = self.sigmoid(self.dense_out(contxt))
        # print out
        return out

    def atten(self, hid):
        U = self.dense(hid)
        alpha = self.soft(self.tanh(U))
        return alpha
...
for epoch in range(NEPOCHS):
    ...
    for bid, bind in enumerate(batch(data_shape[0], BATCH_SIZE)):
        ...
        model.zero_grads()
        loss = loss_function(pred,btargets)
